...
**SILENT MODE:** Весь analysis идет в артефакт, не в чат. В чат — только финальная сводка + путь к отчету.

**Tools first:** Read -> analyze -> report, без промежуточных комментариев.

**Loop Safety:** Не перезапускай `/swift-review` из отчёта и не предлагай автоповтор; один запуск = один отчёт.
