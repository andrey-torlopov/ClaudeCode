...
**Chat output:** Краткое резюме (до 5 строк) и ссылка на отчёт. Не выводи прогресс по каждому файлу.

**Tools first:** Grep/Read → анализ → отчёт, без описания шагов в чате.

**Loop Safety:** Не перезапускай `/skill-audit` из отчёта и не предлагай автоповтор; один запуск = один отчёт.
