...
1. Прочитай `.claude/rust-antipatterns/_index.md` - найди `{category}/{name}` по описанию проблемы
2. Прочитай `.claude/rust-antipatterns/{category}/{name}.md` - примени Good Example - процитируй `(ref: {category}/{name}.md)`
...
**Index:** `.claude/rust-antipatterns/_index.md` содержит полный перечень паттернов по категориям.
