...
- Referenced файл ДОЛЖЕН существовать.

### context
- **Варианты:**
  - `fork` - изолированный контекст (Process Isolation, рекомендуется для большинства skills)
  - `inherit` - унаследованный контекст (по умолчанию)
- Если не указан - наследуется от вызывающего
...
---
name: rust-review
description: Глубокий code review Rust-кода с фокусом на memory safety, concurrency и Rust conventions. Используй для ревью модулей и PR. Не используй для анализа зависимостей.
...
---
name: init-project
description: Генерирует COMMON.md и anchor-файлы для Rust проекта на основе анализа репозитория. Используй для нового проекта без prompt pack. Не используй если core context уже настроен.
...
description: Анализирует Cargo-зависимости проекта на актуальность, уязвимости и конфликты. Используй перед обновлением зависимостей. Не используй для анализа кода.
