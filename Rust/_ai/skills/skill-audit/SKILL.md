...
**Structured Output Priority:** Весь analysis идет в артефакт (MD/HTML), не в чат.

**Chat output (ограничения):**
- Brief Summary: max 5 строк (что нашли, сколько, итог)
- Findings table: max 15 строк (топ по severity)
- Полный отчет: `Полный отчет: {path}` + открыть файл
...
Для каждого SKILL.md - `wc -l`. Порог берется из `init-skill/references/validation-checklist.md` (секция "Структура", поле SKILL.md <= N строк):
...
Grep: `Self-Review`, `self_review`, `_self_review.md`, шаблоны отчетов с `Scorecard`.
...
2. Grep в каждом SKILL.md по ключевым словам стека (tokio, serde, reqwest, etc.)
...
Найти code blocks (```rust, ```toml, etc.) в SKILL.md. Подсчитать строки в каждом.
...
- Рекомендация: заменить на однострочники, подробности -> `.ai/patterns/*.md` или skill-specific references/
...
3. Проверить Skill Completion Protocol на ссылки удаленных паттернов
...
Записать полный отчет в `audit/skill-audit-report.md` (таблица находок + Summary).
...
**Если нашел ошибку в аудите -> исправь.**
