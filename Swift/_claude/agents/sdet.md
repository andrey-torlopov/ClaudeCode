...
1. Прочитай `.claude/swift-patterns/_index.md` - найди `{category}/{name}` по описанию проблемы
2. Прочитай `.claude/swift-patterns/{category}/{name}.md` - примени Good Example - процитируй `(ref: {category}/{name}.md)`
...
**Index:** `.claude/swift-patterns/_index.md` содержит полный перечень паттернов по категориям.
