...
- Vague Feedback: не "Код выглядит странно", а "В строке 45 Thread.sleep() запрещён (ref: platform/flaky-sleep-tests.md)"
...
- **CRITICAL** - compilation fail, security hole, data loss, retain cycle, data race, logic deviation from spec -> **CRITICAL WARNING**, строгая рекомендация к исправлению
- **MAJOR** - performance issue, нарушение паттерна из patterns/, force unwrap в production, missing Sendable -> **MAJOR WARNING**, рекомендация в отчёте
