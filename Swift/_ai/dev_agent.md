...
### Производительность (из RnD)

1. **URL + resourceValues > attributesOfItem(atPath:)** - запрашивай только нужные ключи
2. **enumerator(at:includingPropertiesForKeys:) > subpathsOfDirectory** - prefetch атрибутов
3. **[UInt8]/Data > String** - для сравнения секретов, парсинга бинарных данных
4. **Set/Dictionary > String.contains** - O(1) поиск вместо линейного
5. **URL > String** - для путей файловой системы
6. **Мемоизация с TTL** - для дорогих FS-операций (free space, directory size)
7. **Один мир bridging** - не чередуй NSString/String в tight loop
8. **utf8 view / bytes** - для посимвольной обработки в горячем пути
