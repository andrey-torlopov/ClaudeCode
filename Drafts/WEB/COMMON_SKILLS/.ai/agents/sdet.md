...
1. Прочитай `.ai/swift-antipatterns/_index.md` - найди `{category}/{name}` по описанию проблемы
2. Прочитай `.ai/swift-antipatterns/{category}/{name}.md` - примени Good Example - процитируй `(ref: {category}/{name}.md)`
...
**Index:** `.ai/swift-antipatterns/_index.md` содержит полный перечень паттернов по категориям.
