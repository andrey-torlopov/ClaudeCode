...
|---------------------|----------|
| <= threshold | OK |
| threshold+1 ... threshold x 1.1 | WARNING |
| > threshold x 1.1 | CRITICAL |
...
- Рекомендация: заменить на "ИСПРАВЬ конфигурацию, провалидируй"

### Check 4: Tech Stack Duplication

1. Прочитай CLAUDE.md -> найди Tech Stack
2. Grep в каждом SKILL.md по ключевым словам стека (Docker, Terraform, Kubernetes, Prometheus, etc.)
...
Найти code blocks (```bash, ```yaml, ```hcl, etc.) в SKILL.md. Подсчитать строки в каждом.
...
- Рекомендация: заменить на однострочники, подробности -> skill-specific references/
...
3. Проверить ссылки на `_ai/devops-patterns/` и `_ai/agents/`
