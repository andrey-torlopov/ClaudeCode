...
4. **Inventory Step:**
   ...
   - Для каждого файла определить:
     - Path (относительный)
     - Line count (`wc -l`)
     - Type classification (по правилам из `references/check-rules.md` п. 1)
...
1. Загрузить пороги из `references/check-rules.md` п. 1
...
Загрузить паттерны из `references/check-rules.md` п. 2.
...
1. Нормализовать по правилам из `references/check-rules.md` п. 5
...
1. Определить категорию контента по `references/check-rules.md` п. 3
...
- Загрузить маркеры из `references/check-rules.md` п. 4
