...
|-----------|-------------|
| Сгенерируй конфигурацию | Вы должны сгенерировать конфигурацию |
...
1. **Trigger-фразы** - что скажет пользователь, чтобы вызвать skill?
   Пример: "проанализируй Dockerfile", "проверь Terraform-модуль"
...
**Critical rules:**
- Папка: только kebab-case (`my-skill` - ok, `My_Skill` - bad)
